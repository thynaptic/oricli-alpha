...
A comprehensive testing framework for Mavaia that enables curriculum-based evaluation through progressive difficulty testing and selective testing via an interactive menu system.
...
The framework is included with Mavaia Core. Install dependencies:
...
```bash
mavaia-curriculum-test --full
```

#### Interactive Selection

```bash
mavaia-curriculum-test --select
```

#### Direct Parameter Selection

```bash
mavaia-curriculum-test --direct \
  --level k5 \
  --subject math \
...
from mavaia_core.evaluation.curriculum.models import MemoryContinuityMode, SafetyPosture
