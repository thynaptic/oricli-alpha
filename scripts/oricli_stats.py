...
def resolve_path(p_str, root, env_name):
    ...
    if env_name == "Local":
        ...
        remote_bases = [
            "/workspace/oricli/oricli_core/models/neural_text_generator/curriculum",
            "/workspace/oricli/oricli_core/models/neural_text_generator/runs"
        ]
        ...
        if str(p).startswith("/workspace/oricli/"):
            return root / p.relative_to("/workspace/oricli")
            
    return p

def generate_layout(root, env_name):
    # Detect curriculum path - Check both local and remote-sync
    local_curriculum = root / "oricli_core" / "models" / "neural_text_generator" / "curriculum"
...
def main():
    ...
    if os.path.exists("/workspace/oricli"):
        root, env_name = Path("/workspace/oricli"), "RunPod (Remote)"
    else:
