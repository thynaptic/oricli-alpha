...
class ModuleRegistry:
    ...
    def get_modules_dir(cls) -> Optional[Path]:
        """Get the modules directory - only oricli_core/brain/modules/"""
        if cls._modules_dir is not None:
            return cls._modules_dir
        
        # Only use modules directory within oricli-core package
        # This ensures we only discover actual brain modules
        package_dir = Path(__file__).parent.parent.parent
        brain_modules_dir = package_dir / "oricli_core" / "brain" / "modules"
    ...
    def _discover_modules_internal(
        ...
        if modules_dir is None:
            ...
            if modules_dir is None:
                # Only use oricli_core/brain/modules/ - no fallbacks
                if verbose:
                    print(f"[ModuleRegistry] Modules directory not found: oricli_core/brain/modules/", file=sys.stderr)
                return (0, 0)
        else:
        ...
        for module_file in python_files:
            try:
                ...
                try:
                    from oricli_core.brain import base_module as package_base_module
    ...
    def get_module(
        ...
        if name not in cls._instances:
            ...
            try:
                ...
                try:
                    from oricli_core.brain.wrapper import MetricsTrackingWrapper
