...
def _lazy_import_storage():
    ...
    if not STORAGE_AVAILABLE:
        try:
            from oricli_core.brain.state_storage.base_storage import BaseStorage as BS, StorageConfig as SC
            from oricli_core.brain.state_storage.memory_storage import MemoryStorage as MS
            BaseStorage = BS
            StorageConfig = SC
            MemoryStorage = MS
...
def _lazy_import_memory_graph():
    ...
    if not MEMORY_GRAPH_AVAILABLE:
        try:
            from oricli_core.brain.modules.memory_graph import MemoryGraph as MG
            MemoryGraph = MG
