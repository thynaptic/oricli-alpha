...
- [ ] Implement `oricli_core/brain/modules/adversarial_auditor.py`.
...
- [ ] Update `oricli_core/brain/modules/cognitive_generator.py` to call the Auditor after the `pathway_architect` but before the `graph_executor`.
- [ ] Implement the "Audit Rejection" loop: if audit fails, re-call architect with `adversarial_constraints`.

## Phase 4: Feedback Loop
- [ ] Create `oricli_core/data/red_team_lessons.jsonl`.
