...
## Objective
To quantify and improve Mavaia's ability to select and execute tools correctly, safely, and idiomatically by creating a self-updating evaluation and training loop.

## Core Components

1. **The Registry Inspector**:
   - Introspects `mavaia_core/services/tool_registry.py` to extract all available tool names, descriptions, and JSON schemas.
...
3. **The Efficacy Grader**:
   - Compares Mavaia's generated tool call against the target schema.
   - Metrics:
...
5. **The Tool Daemon (`mavaia_tool_daemon.py`)**:
...
3. **Execute**: Run Mavaia through the scenarios.
