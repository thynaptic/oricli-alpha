...
// CanvasConstitution enforces production-readiness for Canvas-mode artifact generation.
// Language-agnostic (React/JSX, HTML, Python, Go snippets, shell, etc.).
// Ported from pkg/reform — prompt inject only.
	...
	return &CanvasConstitution{
		Principles: []CodePrinciple{
			...
			},
			{
				...
				Guideline:   "Do not add <script src='https://...'>, unpkg/jsDelivr CDN imports, or fetch() calls to external services not already established. All imports must reference packages already present. Never introduce network egress to unknown endpoints.",
			},
			{
...
// PromptForSurface returns the constitution block for a chat surface.
// canvas → CanvasConstitution; code/dev → CodeConstitution; else empty.
func PromptForSurface(canvasMode, codeContext bool) string {
	switch {
	case canvasMode:
		return NewCanvasConstitution().GetSystemPrompt()
	case codeContext:
		return NewCodeConstitution().GetSystemPrompt()
	default:
		return ""
	}
}
