...
// Document is one BM25-indexed chunk.
...
// BM25Index is an in-memory Okapi BM25 index (no embeddings).
...
func (idx *BM25Index) Search(query string, limit int, namespace string) []Result {
	idx.mu.RLock()
	defer idx.mu.RUnlock()
	...
	for docID, score := range scores {
		...
		results = append(results, Result{
			ID: docID, Content: d.Content, Metadata: meta, Score: score,
		})
	}
...
func (idx *BM25Index) addDocLocked(doc Document) {
...
func (idx *BM25Index) removeDocLocked(doc Document) {
...
func normalizeBM25Results(in []Result) []Result {
	if len(in) == 0 {
		return nil
	}
	maxScore := 0.0
