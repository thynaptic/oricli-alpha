...
const defaultFingerprintMaxBytes = int64(256 * 1024)

// IncrementalFileRecord tracks one indexed file for incremental re-ingest.
...
// IncrementalManifest is the on-disk incremental index bookkeeping file.
...
// ManifestPathForRoot returns a stable manifest path under memoryDir/incremental/.
func ManifestPathForRoot(memoryDir, root string) string {
	...
	return filepath.Join(base, "incremental", hex.EncodeToString(sum[:8])+".json")
}

func LoadIncrementalManifest(path string) (IncrementalManifest, error) {
	out := IncrementalManifest{Version: 1, Files: map[string]IncrementalFileRecord{}}
...
// ComputeFastFileFingerprint returns size, mtime unix, and a cheap content hash.
